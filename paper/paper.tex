...
Addressing these problems, we propose a data-driven, syntax-based approach to this task and put it to the test in a simple grammar-based translation pipeline.
\end{abstract}
...
While converting them to GF ASTs is \texttt{gf-ud}'s core functionality, to build a compilable GF translation lexicon it is necessary to turn such trees into grammar rules.
...
The results of this comparison are summarized and compared with those obtained on the sample of PUD treebanks in Table \ref{raw_fast}.
...
The percentage of correct alignments, however, stays significantly higher than that obtained with \texttt{fast\_align} in the previous experiment, even when the model was trained on the full PUD corpus: even though percentages seem similar for English-Swedish, it must be taken into account that the CSE corpus is roughly half the size of the PUD corpus.
