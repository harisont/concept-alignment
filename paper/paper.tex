...
\maketitle
\begin{abstract}
In grammar-based domain-specific MT, constructing high-quality translation lexica is a crucial but time consuming task requiring significant linguistic knowledge. 
...
Addressing these problems, we propose a grammar-based approach to this task and put it to the test in a simple translation pipeline.
\end{abstract}
...
% how it is done right now and issues
...
Our basic algorithm is sketched in th pseudocode below.
...
% input
Here, the input consists of a list of priority-sorted \textit{alignment criteria}, i.e. rules to determine whether two dependency trees should be aligned with each other which will be discussed in greater detail in Section \ref{criteria}, and a pair of sentences to align represented as UD trees.
...
A relevant implementative detail is that, in this context, the head of a tree is not simply defined as its root. Instead, if the root is part of a compound written as two or more separate words or a verb with auxiliaries, the root nodes of the corresponding subtrees are also considered part of it.
...
% why list criteria 
While alignment criteria are customizable, to allow for a better understanding of the extraction algorithm described in the above, we describe the ones that the current implementation utilizes by default.
...
As noted in the above, CoNNL-U files also provide information on the grammatical categories of each word, represented as Universal POS tags \cite{petrov-etal-2012-universal}. 
...
An ubiquitous example of is that of adverbs rendered as obliques, such as in the English-Italian pair $\langle$\textit{She studies \textbf{consistently}, Lei studia \textbf{con costanza}}$\rangle$.
\textit{Structural divergences}, where a core argument of a sentence in the source language are rendered as obliques in the source language (as in the English-Swedish pair $\langle$\textit{I told \textbf{him}, Jag berättade \textbf{för honom}.}$\rangle$), are also frequently encountered.

% TODO: add example (Herb?)
...
We first discuss the data used for such evaluation. 
