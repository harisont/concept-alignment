...
2. pad the trees $\to$ perfectly aligned trees
   ![padding](figures/padded_rtrees.png)
3. extract alignments:
   - subtrees: $\langle$_she missed the boat, ha perso il treno_$\rangle$, $\langle$_the boat, il treno_$\rangle$, $\langle$_the,il_$\rangle$
   - heads:$\langle$_missed,perso_$\rangle$, $\langle$_boat,treno_$\rangle$

## Proposed improvements
- multiple alignment criteria $\to$ no perfect alignment
   - label matching
   - POS-equivalence
   - known alignment
   - known divergence
...
## Basic criteria
- __label matching__ (original criterion): trees in matching context are aligned if they have the same UD label  
...
## Divergences
__Divergence__: systematic cross-linguistic distinctions.

- categorial
  - _Gioara listens **distractedly**_ VS _Gioara lyssnar **distraherad**_
  - _Herbert completed his **doctoral** thesis_ VS _Herbert ha completato la sua tesi **di dottorato**_
- conflational
  - _Filippo is interested in **game development**_ VS _Filippo är intresserad av **spelutveckling**_
- structural
  - _I called **Francesco**_ VS _Ho telefonato **a Francesco**_
- head swapping
  - _Anna **usually** goes for walks_ VS _Anna **brukar** promenera_
- thematic
...
- extremely useful when alignment is perfect, like $\langle$_Claudio eats a banana,Claudio mangia una banana_$\rangle$
  ...
  - $\langle$_banana,banana_$\rangle$
- many problematic cases
  - some types of divergences $\to$ do not always align heads
...
- domain-specific concepts
  - $\langle$_skills, färdigheter, capacità_$\rangle$, $\langle$_exam, tentamen, prova_$\rangle$...
  - $\langle$_the aim of the course, syftet med kursen, l'obiettivo del corso_$\rangle$
...
- again via one of `gf-ud`'s modules
  - requires: __extraction grammar__, __morphological dictionaries__
...
## Evaluation: strategy
- small course plans corpora $\to$ 2 bilingual lexica instead of a trilingual one
...
- max score:
  ...
  - $\langle$_this lab is more difficult than the exam, den här laborationen är svårare än tentamen_$\rangle$
- min score:
  - $\langle$_the test is oral, la prova è dura_$\rangle$
